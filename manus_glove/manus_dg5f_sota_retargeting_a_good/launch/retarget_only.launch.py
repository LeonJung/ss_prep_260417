...
Use when Manus driver + DG side already run elsewhere. Typical use:
  ros2 launch manus_dg5f_sota_retargeting_a_good retarget_only.launch.py hand_side:=right
  ros2 launch manus_dg5f_sota_retargeting_a_good retarget_only.launch.py hand_side:=left
"""
...
def generate_launch_description():
    ...
    cfg_yaml = PathJoinSubstitution([
        FindPackageShare("manus_dg5f_sota_retargeting_a_good"), "config", yaml_name
    ])

    node_name = PythonExpression([
        '"manus_dg5f_sota_retargeting_a_good_" + "', hand_side, '"'
    ])
